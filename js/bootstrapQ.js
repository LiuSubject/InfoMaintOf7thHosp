...
$.fn.bstree = function(options){
	...
	bootstrapQ.tree.init($(this).attr('id'));
};
bootstrapQ.tree.sub = function(tree, opt){
	...
	if(tree){
		...
		if((opt.open && tree.pid < 0) || (typeof children === 'undefined')) {
			res += ' glyphicon-minus"></span>';
		}else{
			res += ' glyphicon-plus"></span>';
		}
		if(opt.checkbox){
			res += '<input type="checkbox" style="margin-left:6px; margin-right:1px;" class="treecheckbox '+opt.checkClass+'" value="' + tree.id + '" tName="'+tree.name+'"';
						/*if(tree.pid > 0){
				res += '<input type="checkbox" class="treecheckbox '+opt.checkClass+'" value="' + tree.id + '" tName="'+tree.name+'"';
			}else{
				res += '<input type="checkbox" class="treecheckbox" value="' + tree.id + '" tName="'+tree.name+'"';
			}*/
			if(tree.checked){
				res += 'checked';
			}
		...
		if(tree.pid >= 0){
			if(opt.add){
				res += '&nbsp;&nbsp;<span class="label label-primary bstreeadd">\u6dfb\u52a0\u5b50\u8282\u70b9</span>';
			}
			if(opt.edit){
				res += '&nbsp;&nbsp;<span class="label label-primary bstreeedit">\u4fee\u6539</span>';
			}
			if(opt.del){
				res += '&nbsp;&nbsp;<span class="label label-danger  bstreedel">\u5220\u9664</span>';
			}
		}else{
			if(opt.add){
				res += '&nbsp;&nbsp;<span class="label label-primary bstreeadd">\u6dfb\u52a0\u5b50\u8282\u70b9</span>';
...
bootstrapQ.tree.addp = function(){
	bootstrapQ.modaloptions.foot = false;
	bootstrapQ.dialog({
		url 	:  bootstrapQ.tree.url + '=add&pid=' + $(this).parent().qdata().id,
		title 	: '\u6dfb\u52a0\u5b50\u83dc\u5355',
		okbtn 	: '\u4fdd\u5b58'
//	}, bootstrapQ.tree.add);
	}, function(){});
...
bootstrapQ.tree.del = function(){
	bootstrapQ.modaloptions.foot = true;
	var nodeId = $(this).parent().qdata().id;
	bootstrapQ.confirm({
	    title:'提示',
        msg:'确认删除本节点?',
        backdrop:'true'
        ...
        function(){
        	var res = qiao.ajax({url:bootstrapQ.tree.url + '=del&id=' + nodeId});
        	bootstrapQ.msg(res);
        	bootstrapQ.tree.delCallback();
        });
...
bootstrapQ.tree.editp = function(){
	bootstrapQ.modaloptions.foot = false;
	bootstrapQ.dialog({
		url 	: bootstrapQ.tree.url + '=edit&id=' + $(this).parent().qdata().id,
		title 	: '\u4fee\u6539\u83dc\u5355',
		okbtn 	: '\u4fdd\u5b58'
